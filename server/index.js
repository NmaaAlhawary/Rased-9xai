/**
 * Rased — راصد Backend Proxy Server
 *
...
import { AMMAN_ROADS } from './data/ammanRoads.js';
import { JORDAN_BORDERS } from './data/jordanBorders.js';
import { JORDAN_CAMERAS } from './data/jordanCameras.js';
import { JORDAN_HIGHWAYS } from './data/jordanHighways.js';

// Static fallback air quality stations for Jordan (used when OpenAQ is unavailable/empty)
const STATIC_JORDAN_AQ = [
  { id: 1001, name: 'Amman - Downtown', lat: 31.9554, lon: 35.9384, pm25: 18, pm10: 35, no2: 22, aqi: 64, updatedAt: new Date().toISOString() },
  { id: 1002, name: 'Amman - Sweifieh', lat: 31.9685, lon: 35.8825, pm25: 14, pm10: 28, no2: 18, aqi: 52, updatedAt: new Date().toISOString() },
  { id: 1003, name: 'Zarqa Industrial Zone', lat: 32.0686, lon: 36.0886, pm25: 42, pm10: 78, no2: 45, aqi: 118, updatedAt: new Date().toISOString() },
  { id: 1004, name: 'Aqaba Port Area', lat: 29.5182, lon: 35.0054, pm25: 10, pm10: 22, no2: 14, aqi: 38, updatedAt: new Date().toISOString() },
  { id: 1005, name: 'Irbid City Centre', lat: 32.5568, lon: 35.8469, pm25: 22, pm10: 40, no2: 28, aqi: 75, updatedAt: new Date().toISOString() },
  { id: 1006, name: 'Mafraq Governorate', lat: 32.3432, lon: 36.2084, pm25: 8, pm10: 18, no2: 10, aqi: 30, updatedAt: new Date().toISOString() },
  { id: 1007, name: 'Russeifa Industrial', lat: 32.0121, lon: 36.0512, pm25: 55, pm10: 92, no2: 60, aqi: 153, updatedAt: new Date().toISOString() },
  { id: 1008, name: 'Dead Sea Highway', lat: 31.7684, lon: 35.5893, pm25: 6, pm10: 14, no2: 8, aqi: 22, updatedAt: new Date().toISOString() },
];

const app = express();
const PORT = process.env.PORT || 3001;

// Restrict CORS to the declared origin in production; use * for local dev.
...
/** GET /api/earthquakes — Global 2.5_week feed + Eastern Med regional supplement */
app.get('/api/earthquakes', async (_req, res) => {
  try {
    const cached = cache.get('earthquakes');
    if (cached) return res.json(cached);

    // Fetch Middle East / Eastern Mediterranean region only (Dead Sea Transform Fault zone)
    // Bounding box covers Jordan, Palestine, Lebanon, Syria, Turkey, Cyprus, Egypt Sinai
    const starttime = new Date(Date.now() - 5 * 365 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    const regionalRes = await fetch(
      'https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson' +
      '&minmagnitude=2.5&minlatitude=27&maxlatitude=39&minlongitude=32&maxlongitude=43' +
      `&starttime=${starttime}&orderby=time&limit=500`
    );
    ...
    if (regionalRes.ok) {
      ...
      console.log(`[EQ] Middle East regional: ${features.length} events`);
    }

    // Sanitize place names — replace any reference to 'Israel' with 'Palestine'
    for (const f of features) {
      if (f.properties?.place) {
        f.properties.place = f.properties.place.replace(/\bIsrael\b/gi, 'Palestine');
      }
    }
    ...
    const merged = {
      ...
      features,
      metadata: { count: features.length, generated: Date.now() },
    };

    cache.set('earthquakes', merged, 300); // Cache 5 minutes
    res.json(merged);
  } catch (err) {
...
// Static fallback road data loaded from ./data/ammanRoads.js (15 key arteries, Amman metropolitan area)
const AMMAN_BBOX = { south: 31.90, west: 35.82, north: 32.05, east: 36.00 };

/**
 * Check if a requested bbox overlaps with the static Amman data bbox.
 */
function bboxOverlapsAmman(south, west, north, east) {
  return !(east < AMMAN_BBOX.west || west > AMMAN_BBOX.east ||
           north < AMMAN_BBOX.south || south > AMMAN_BBOX.north);
}
...
app.get('/api/traffic/roads', async (req, res) => {
  try {
    ...
    // ── Try Overpass first (real street-level data for any location) ──
    try {
      const roads = await fetchFromOverpass(bbox);
      if (roads.length > 0) {
        console.log(`[TRAFFIC] Overpass: ${roads.length} roads for bbox: ${south},${west},${north},${east}`);
        cache.set(cacheKey, roads, 3600); // cache 1 hour
    ...
    // ── Overpass unavailable — fall back to static Jordan data ──
    if (bboxOverlapsAmman(bbox.south, bbox.west, bbox.north, bbox.east)) {
      const filteredAmman = filterRoadsByBbox(AMMAN_ROADS, bbox.south, bbox.west, bbox.north, bbox.east);
      const filteredHighways = filterRoadsByBbox(JORDAN_HIGHWAYS, bbox.south, bbox.west, bbox.north, bbox.east);
      const filtered = [...filteredAmman, ...filteredHighways];
      console.log(`[TRAFFIC] Static Jordan fallback: ${filtered.length} roads`);
      cache.set(cacheKey, filtered, 86400);
      return res.json({ roads: filtered, cached: false, source: 'static-jordan' });
    }

    const jordanBbox = { south: 29.0, west: 34.8, north: 33.5, east: 39.5 };
    const inJordan = !(bbox.east < jordanBbox.west || bbox.west > jordanBbox.east ||
                       bbox.north < jordanBbox.south || bbox.south > jordanBbox.north);
    if (inJordan) {
      const filteredHighways = filterRoadsByBbox(JORDAN_HIGHWAYS, bbox.south, bbox.west, bbox.north, bbox.east);
      if (filteredHighways.length > 0) {
        console.log(`[TRAFFIC] Static Jordan highways fallback: ${filteredHighways.length} roads`);
        cache.set(cacheKey, filteredHighways, 86400);
        return res.json({ roads: filteredHighways, cached: false, source: 'static-jordan-highways' });
      }
    }
  ...
  try {
    ...
    const sources = [
      ...
      },
      {
        ...
        ttl: 300,
        fetch: async () => {
          // Update lastUpdated timestamps on each serve
          return JORDAN_CAMERAS.map((c) => ({ ...c, lastUpdated: new Date().toISOString() }));
        },
      },
    ];
...
/** GET /api/borders — Jordan border crossing status */
app.get('/api/borders', (_req, res) => {
  try {
    const cached = cache.get('jordan-borders');
    if (cached) return res.json(cached);
    // Return static data — in production, update from a live government feed
    const data = JORDAN_BORDERS.map((b) => ({
      ...b,
      fetchedAt: new Date().toISOString(),
    }));
    cache.set('jordan-borders', data, 300); // Cache 5 minutes
    res.json(data);
  } catch (err) {
    console.error('[BORDERS] Error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

/** GET /api/airquality — Jordan air quality stations via OpenAQ (no key required) */
app.get('/api/airquality', async (_req, res) => {
  try {
    const cacheKey = 'jordan-airquality';
    const cached = cache.get(cacheKey);
    if (cached) return res.json(cached);

    const r = await fetch(
      'https://api.openaq.io/v3/locations?country=JO&limit=50&order_by=lastUpdated&sort=desc',
      { headers: { 'Accept': 'application/json', 'User-Agent': 'Rased-AQ/1.0' } }
    );
    if (!r.ok) throw new Error(`OpenAQ HTTP ${r.status}`);
    const data = await r.json();

    const stations = (data.results || []).map((loc) => {
      // OpenAQ v3 uses `parameters` array (not `sensors`)
      const parameters = loc.parameters || loc.sensors || [];
      const getParam = (param) => {
        const p = parameters.find((p) => (p.parameter?.name || p.parameter || p.name) === param);
        return p?.latest?.value ?? p?.lastValue ?? null;
      };
      const pm25 = getParam('pm25');
      const pm10 = getParam('pm10');
      const no2  = getParam('no2');

      // Simple AQI estimation from PM2.5 (US EPA breakpoints, simplified)
      let aqi = 0;
      if (pm25 != null) {
        if (pm25 <= 12)   aqi = Math.round((pm25 / 12) * 50);
        else if (pm25 <= 35.4) aqi = Math.round(50 + ((pm25 - 12) / 23.4) * 50);
        else if (pm25 <= 55.4) aqi = Math.round(100 + ((pm25 - 35.4) / 20) * 50);
        else aqi = Math.round(150 + ((pm25 - 55.4) / 144.5) * 100);
      } else if (pm10 != null) {
        aqi = Math.round(Math.min((pm10 / 154) * 100, 200));
      }

      return {
        id: loc.id,
        name: loc.name || loc.locality || 'Unknown',
        lat: loc.coordinates?.latitude ?? 0,
        lon: loc.coordinates?.longitude ?? 0,
        pm25,
        pm10,
        no2,
        aqi,
        updatedAt: loc.datetimeLast?.utc || new Date().toISOString(),
      };
    }).filter((s) => s.lat !== 0 && s.lon !== 0);

    console.log(`[AQ] Fetched ${stations.length} Jordan air quality stations`);

    // If OpenAQ returned no useful data, serve static representative stations
    const finalStations = stations.length > 0 ? stations : STATIC_JORDAN_AQ;
    cache.set(cacheKey, finalStations, 300); // Cache 5 minutes
    res.json(finalStations);
  } catch (err) {
    console.error('[AQ] Error:', err.message);
    // Return static fallback on failure
    res.json(STATIC_JORDAN_AQ);
  }
});

/** GET /api/weather — Live weather for major Jordan cities via Open-Meteo (free, no key) */
const JORDAN_CITIES = [
  { id: 'amman',    name: 'Amman',    lat: 31.9539, lon: 35.9106 },
  { id: 'aqaba',    name: 'Aqaba',    lat: 29.5321, lon: 35.0063 },
  { id: 'irbid',    name: 'Irbid',    lat: 32.5568, lon: 35.8469 },
  { id: 'zarqa',    name: 'Zarqa',    lat: 32.0686, lon: 36.0886 },
  { id: 'salt',     name: 'Salt',     lat: 32.0392, lon: 35.7275 },
  { id: 'madaba',   name: 'Madaba',   lat: 31.7167, lon: 35.8000 },
  { id: 'jerash',   name: 'Jerash',   lat: 32.2742, lon: 35.8997 },
  { id: 'karak',    name: 'Karak',    lat: 31.1834, lon: 35.7024 },
  { id: 'petra',    name: 'Petra',    lat: 30.3285, lon: 35.4444 },
  { id: 'wadi-rum', name: 'Wadi Rum', lat: 29.5760, lon: 35.4228 },
];

app.get('/api/weather', async (_req, res) => {
  try {
    const cacheKey = 'jordan-weather';
    const cached = cache.get(cacheKey);
    if (cached) return res.json(cached);

    // Build a single batched Open-Meteo request using the multi-location format
    const latitudes  = JORDAN_CITIES.map((c) => c.lat).join(',');
    const longitudes = JORDAN_CITIES.map((c) => c.lon).join(',');
    const url =
    ...
    const r = await fetch(url, { headers: { 'User-Agent': 'Rased-Weather/1.0' } });
    ...
    const stations = JORDAN_CITIES.map((city, i) => {
      const d = results[i];
    ...
    console.log(`[WEATHER] Fetched weather for ${stations.length} Jordan cities`);
    cache.set(cacheKey, stations, 600); // Cache 10 minutes
    res.json(stations);
  } catch (err) {
    console.error('[WEATHER] Error:', err.message);
    // Static fallback with realistic seasonal values
    const fallback = JORDAN_CITIES.map((c) => ({
      id: c.id, name: c.name, lat: c.lat, lon: c.lon,
      temperature: c.lat > 31 ? 22 : 28, // cooler in north, warmer in south
      weatherCode: 1, windSpeed: 15, humidity: 45,
 ...
 */
async function collectAISBurst(apiKey, durationMs = 12000) {
...
app.get('/api/ships', async (req, res) => {
  try {
    ...
    if (!allVessels) {
      ...
      const raw = await collectAISBurst(apiKey, 12000);
...
// ─── Route registry + WebSocket: local dev only ─────────────────
// On Vercel, setInterval doesn't persist across invocations and
// WebSocket upgrades aren't supported in standard serverless functions.
...
async function startFlightPolling(bbox) {
  if (!wss) return; // no-op on Vercel serverless
