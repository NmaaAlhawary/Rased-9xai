...
// stations = ISS, CSS, Tiangong (shown as 'iss' or 'other' in SatelliteLayer).
// weather  = NOAA, Meteosat, GOES etc. (shown as 'other').
const SATELLITE_GROUPS = [
  '/api/satellites?group=stations',
  '/api/satellites?group=weather',
] as const;
    ...
    for (let attempt = 0; attempt <= TLE_FETCH_RETRIES; attempt++) {
      try {
        ...
        const responses = await Promise.all(
          SATELLITE_GROUPS.map((url) => fetch(url))
        );
        for (const res of responses) {
          if (!res.ok) throw new Error(`Satellite proxy HTTP ${res.status} (${res.url})`);
        }
        const texts = await Promise.all(responses.map((r) => r.text()));
        const tleEntries = texts.flatMap(parseTLEText);
        ...
        setFeedItems([
          {
            ...
            type: 'satellite',
            message: `${tleEntries.length} satellites tracked (${SATELLITE_GROUPS.length} groups)`,
          },
        ]);

        console.info(`[SAT] Loaded ${tleEntries.length} TLEs from ${SATELLITE_GROUPS.length} groups (attempt ${attempt + 1})`);
