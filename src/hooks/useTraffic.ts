...
export interface RoadSegment {
  ...
  /** Congestion score 0–1 (0 = free flow, 1 = standstill). Computed client-side. */
  congestion?: number;
}
...
export function useTraffic(
  ...
  // Pre-computed road lookup map — updated whenever roads change to avoid O(n×m) per frame
  const roadMapRef = useRef<Map<string, RoadSegment>>(new Map());
  // When the layer is toggled ON, skip the camera-movement debounce so roads load immediately
  const skipDebounceRef = useRef(false);

  /** Only fetch when the camera is close enough to see streets (below 80 km altitude) */
  const shouldFetchRoads = altitude < 80_000;

  /** Calculate bounding box from camera position and altitude.
   * Capped at 0.04° (~4.4 km) so Overpass queries stay fast and return detailed streets.
   */
  const calculateBbox = useCallback((lat: number, lon: number, alt: number) => {
    // Scale grows with altitude but capped tightly so we always get street-level detail
    const scale = Math.max(0.005, Math.min(alt / 500000, 0.04));
    return {
      south: lat - scale,
      west: lon - scale,
      north: lat + scale,
  ...
  /** Round lat/lon to ~1km grid to prevent re-fetches on tiny pans.
   * Altitude is intentionally excluded — roads don't change with zoom level.
   */
  const bboxKey = useCallback((lat: number, lon: number) => {
    const precision = 2; // ~1.1km precision
    return `${lat.toFixed(precision)},${lon.toFixed(precision)}`;
  }, []);

  // When the layer is switched ON, clear the bbox cache and mark that the next effect
  // invocation should fire without the camera-movement debounce.
  useEffect(() => {
    if (!enabled) return;
  ...
  // NOTE: altitude is kept in deps only to gate shouldFetchRoads (zoom-out disable).
  // The bboxKey intentionally excludes altitude so zooming doesn't cancel in-flight
  // fetches. An 800ms debounce further prevents fetch spam during camera movement.
  useEffect(() => {
    ...
    const currentKey = bboxKey(latitude, longitude);
    ...
    // On the first enable event skip the debounce entirely so roads appear without
    // waiting for the camera to settle.  For subsequent camera-pan re-fetches the
    // 800ms debounce remains active to prevent request spam.
    const debounceDelay = skipDebounceRef.current ? 0 : 800;
    skipDebounceRef.current = false;

    // Debounce: wait for camera stillness before firing the actual HTTP request.
    // Altitude changes (zooming) continuously reset this timer, so the fetch only
    // fires once the camera settles — not on every frame during a zoom gesture.
    const debounceTimer = setTimeout(() => {
      ...
      const fetchRoads = async () => {
        ...
        try {
          ...
          const rawRoads = data.roads as RoadSegment[];
          // Enrich each road with a simulated congestion score (time-of-day + road class)
          const roadData = rawRoads.map((r) => ({ ...r, congestion: computeCongestion(r) }));
  ...
  // Keep road lookup map in sync with roads state
  useEffect(() => {
    roadMapRef.current = new Map(roads.map((r) => [r.id, r]));
  }, [roads]);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      vehiclesRef.current = vehiclesRef.current.map((vehicle) => {
        const road = roadMapRef.current.get(vehicle.roadId); // O(1) lookup
...
/**
 * Compute a congestion score (0–1) for a road segment.
 *
 * Factors:
 *  - Time of day: rush hours (07–09, 17–20) → high congestion
 *  - Road class: motorways & trunk roads carry the most traffic
 *  - Per-road variation: a stable pseudo-random offset seeded on the road ID
 *
 * Returns 0 (free flow) … 1 (standstill).
 */
function computeCongestion(road: RoadSegment): number {
  const hour = new Date().getHours();

  // Time-of-day factor
  let timeFactor: number;
  if (hour >= 7 && hour <= 9)        timeFactor = 0.90; // morning rush
  else if (hour >= 17 && hour <= 20) timeFactor = 0.85; // evening rush
  else if (hour >= 12 && hour <= 14) timeFactor = 0.55; // lunch
  else if (hour >= 22 || hour <= 5)  timeFactor = 0.10; // night
  else                               timeFactor = 0.35; // off-peak

  // Base congestion by road class (higher class = more traffic load)
  ...
  const base = classBase[road.highway] ?? 0.30;

  // Stable per-road pseudo-random variation (±0.15) so each street looks different
  let hash = 5381;
  for (let i = 0; i < road.id.length; i++) {
    hash = ((hash << 5) + hash + road.id.charCodeAt(i)) | 0;
  }
  const variation = ((Math.abs(hash) % 31) - 15) / 100; // −0.15 … +0.15

  return Math.max(0, Math.min(1, base * timeFactor + variation));
}
