...
export default function TrafficLayer({
  ...
  useEffect(() => {
    ...
    newRoads.forEach((road) => {
      ...
      // Use congestion score when available, otherwise fall back to road-class colour
      const color = road.congestion != null
        ? getCongestionColor(road.congestion)
...
/**
 * Map a 0–1 congestion score to a traffic colour:
 *   green (free) → yellow (moderate) → orange (slow) → red (heavy)
 * Mirrors the industry-standard traffic-map palette used by Google Maps / Waze.
 */
function getCongestionColor(congestion: number): Color {
  if (congestion < 0.25) return Color.fromCssColorString('#00BB44').withAlpha(0.95); // free flow
  if (congestion < 0.50) return Color.fromCssColorString('#FFC200').withAlpha(0.95); // moderate
  if (congestion < 0.75) return Color.fromCssColorString('#FF6600').withAlpha(0.98); // slow
  return Color.fromCssColorString('#CC0000').withAlpha(1.0);                         // heavy / standstill
}

/**
 * Fallback colour for roads that have no congestion score (road-class palette).
 */
function getRoadColor(roadClass: string): Color {
  const colorMap: Record<string, Color> = {
    motorway:    Color.fromCssColorString('#CC0000').withAlpha(1.0),
    trunk:       Color.fromCssColorString('#FF6600').withAlpha(1.0),
    primary:     Color.fromCssColorString('#FFC200').withAlpha(1.0),
    secondary:   Color.fromCssColorString('#FFC200').withAlpha(0.95),
    tertiary:    Color.fromCssColorString('#00BB44').withAlpha(0.9),
    residential: Color.fromCssColorString('#00BB44').withAlpha(0.85),
  };
  return colorMap[roadClass] ?? Color.fromCssColorString('#888888').withAlpha(0.85);
}
